...
export function RunsScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    return runs.filter(
      (run) =>
        ...
        run.answer.toLowerCase().includes(lower) ||
        run.effective_tier.toLowerCase().includes(lower) ||
        (run.selected_mode ?? "").toLowerCase().includes(lower),
    );
  }, [runs, search]);

  useEffect(() => {
    if (!selectedId && filtered[0]) setSelectedId(filtered[0].run_id);
  }, [filtered, selectedId]);

  // When filter changes, reset selection to first result
  useEffect(() => {
  ...
  return (
    <div className="page-grid animate-fade-in">
      <PageHeader
        eyebrow="Runs"
        title="Audit and trust history"
        description="Search prior executions, inspect routing, review answers, and verify evidence."
      />
      {runsQuery.isLoading ? (
        ...
        <EmptyState
          ...
          title="No runs yet"
          description="Runs appear after you ask questions through an agent."
        />
      ) : (
        <div className="grid gap-6 xl:grid-cols-[20rem_1fr]">
          <Card variant="glass" className="flex max-h-[calc(100vh-14rem)] flex-col overflow-hidden rounded-2xl">
            <CardHeader
              title={`Run list${search ? ` (${filtered.length})` : ""}`}
              action={
                <div className="glass flex h-8 w-full items-center gap-2 rounded-full px-3">
                  <Search className="h-3.5 w-3.5 text-muted-foreground/40" />
                  <input
                    className="flex-1 bg-transparent text-xs text-foreground outline-none placeholder:text-muted-foreground/40"
                    placeholder="Search queries, answers, modes…"
                    value={search}
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      run.run_id === selectedId
                        ...
                        : "border-border/10 bg-secondary/5 hover:bg-secondary/15",
                    )}
                  >
                    <p className="line-clamp-2 text-sm font-medium text-foreground">{run.query}</p>
          ...
          {selected ? (
            <div className="grid content-start gap-6">
              <Card variant="glass" className="rounded-2xl">
                <CardHeader title={selected.query} eyebrow={selected.run_id.slice(0, 8)} />
                <p className="text-sm leading-relaxed text-foreground/80">{selected.answer}</p>
                <div className="mt-5 grid gap-3 rounded-xl border border-border/10 bg-secondary/8 p-4 sm:grid-cols-3">
                  ...
                  <DetailRow
                    label="Fallback"
                    value={selected.provider_fallback_used ? `from ${selected.provider_fallback_from}` : "None"}
                  />
                </div>
              </Card>
